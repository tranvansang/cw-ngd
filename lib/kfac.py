...
class KFACOptimizer(optim.Optimizer):
	def __init__(self,
		...
		self.CovAHandler = _ComputeCovA()
		self.CovGHandler = _ComputeCovG()
	...
	def _save_input(self, module, input):
		if torch.is_grad_enabled() and self.steps % self.TCov == 0:
			...
			_update_running_stat(aa, self.m_aa[module], self.stat_decay)
	...
	def _save_grad_output(self, module, grad_input, grad_output):
		...
		if self.acc_stats and self.steps % self.TCov == 0:
			...
			_update_running_stat(gg, self.m_gg[module], self.stat_decay)
	...
	def _kl_clip_and_update_grad(self, updates, lr):
		...
		nu = min(1.0, math.sqrt(self.kl_clip / vg_sum)) if self.kl_clip > 0 else 1.0
...
def _update_running_stat(aa, m_aa, stat_decay):
...
class _ComputeMatGrad:
	...
	def conv2d(input, grad_output, layer):
		...
		with torch.no_grad():
			...
			grad_output = _try_contiguous(grad_output).view(grad_output.size(0), -1, grad_output.size(-1))
			# b * hw * out_c
			if layer.bias is not None:
				input = torch.cat([input, input.new(input.size(0), 1).fill_(1)], 1)
